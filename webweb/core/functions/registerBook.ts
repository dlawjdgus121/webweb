...
export const saveBookToNotion = async (bookId: string) => {
  ...
  try {
    ...
    // 1) Oracle에서 필요한 필드 조회 (registerBook과 동일 스키마)
    const selectSql = `
    ...
    // 3) Notion 페이지 생성 (표지 이미지는 cover로도 세팅)
    const finalCover =
...
const BOOK_DB_ID = process.env.NOTION_DATABASE_ID!;
    ...
    console.log(`✅ Notion 저장 완료: ${notionRes.id}`);
